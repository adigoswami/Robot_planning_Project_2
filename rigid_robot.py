...
print("Enter the radius of robot: ")
R= int(input("Radius= "))
print("Enter the clearance: ")
C= int(input("clearance= "))
extra = R+C
...
def obstacle_space(x,y,r):
    ...
    q = 0
    if (x<(extra/r)) or (x>(300-extra)/r) or (y<(extra/r)) or (y>(200-extra)/r):
        q=1
    if (y - 1.4*x - (80+extra)/r <= 0) and (y + 1.4*x  -(290+extra)/r <= 0) and (y - 1.2*x -(30+extra)/r >= 0) and (y + 1.2*x -(210+extra)/r >= 0):#rhombus
        q=1
    if (y - (185+extra)/r <= 0) and (y - 13*x + (140-extra)/r <= 0) and (y - x - (100+extra)/r >= 0) and (y - 1.4*x - (80+extra)/r >= 0):#triangle
        q=1
    if (y + 0.6*x - (145+extra)/r >= 0) and (y- 0.6*x + (95-extra)/r <= 0) and (y+ 0.6*x - (175+extra)/r <= 0) and (y- 0.6*x + (125+extra)/r >= 0):#rhombus
        q=1
    
    if (y- (3**(1/2))*x + (134.54482+extra)/r >= 0) and (y - (3**(1/2))*x - (15.455173+extra)/r <= 0) and (y + (1/(3**(1/2)))*x - (84.84827+extra)/r >= 0) and (y + (1/(3**(1/2)))*x - (96.39528096+extra)/r <= 0):#rectangle
        q=1
    if ((x-math.ceil(225/r))**2+math.ceil(y-(150/r))**2-math.ceil((25+extra)/r)**2)<0:#circle
        q=1
    if ((x-math.ceil(150/r))/(math.ceil(40+extra)/r))**2 + ((y - math.ceil(100/r))/(math.ceil(20+extra)/r))**2 - 1 < 0:
        q=1    
    return q
...
if (obstacle_space(goal[0],goal[1],r)==1 or obstacle_space(ndi[0],ndi[1],r)):
    print(obstacle_space(goal[0],goal[1],r))
    print(obstacle_space(ndi[0],ndi[1],r))
    sys.exit("Either goal node or start node lies inside obstacle ")

if (ndi[0] not in range(0,301) or goal[0] not in range(0,301) or ndi[1] not in range(0,201) or goal[1] not in range(0,201)):
...
#Printing the obstacles
    for i in obs_space:
        pygame.draw.rect(screen, Blue, [i[0],200*k-i[1],r*k,r*k])
    pygame.display.flip()
    clock.tick(20)
#Printing the visited nodes
    for i in vc_nd:
        pygame.time.wait(1)
        pygame.draw.rect(screen, White, [i[0],200*k-i[1],r*k,r*k])
        pygame.display.flip()
#Printing the path
    for j in seq[::-1]:
        pygame.time.wait(1)
        pygame.draw.rect(screen, Green, [j[0], 200*k-j[1], r*k,r*k])
        pygame.display.flip()
    pygame.display.flip()
